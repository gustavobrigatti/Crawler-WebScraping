...
class TesteController
{
    ...
    public function store(Request $request, Client $client){
        ...
        if (!file_exists('Carros/'.$request->carro.'.html')){
            $this->criar_cache($request, $client);
        }else{
            //Obtenção da data atual
            $data_atual = new \DateTime();
            //Obtenção da data de criação do arquivo cache
            $data_cache = new \DateTime(date('Y-m-d H:i:s', filectime('Carros/'.$request->carro.'.html')));
            ...
            if ($horas>=1){
                ...
                unlink('Carros/'.$request->carro.'.html');
                $this->criar_cache($request, $client);
            }else{
    ...
    public function criar_cache($request, $client){
        //Início do cache
        ob_start();
        ...
        $this->mensagem .= '<h2>MODELO: '.$request->carro.'</h2>';
        for ($i=1; $i<=2; $i++){
            if ($i==1){
